...
public class FlowLimitFilter extends HttpFilter {
    ...
    private boolean tryCount(String ip){
        synchronized (ip.intern()) {
            if(stringRedisTemplate.hasKey(Const.FLOW_LIMIT_BLOCK + ip)) return false;
    ...
    private boolean limitPeriodCheck(String ip){
        if(stringRedisTemplate.hasKey(Const.FLOW_LIMIT_COUNTER + ip)){
            ...
            stringRedisTemplate.opsForValue().set(Const.FLOW_LIMIT_COUNTER + ip, "1", 3L, TimeUnit.SECONDS);
        }
        return true;
    }
}
