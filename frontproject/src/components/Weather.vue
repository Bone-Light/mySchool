...
<template>
  <div style="height: 150px" v-loading="!data.success" element-loading-text="天气信息正在加载...">
    <div style="display: flex; justify-content: space-between; margin: 10px 20px" v-if="data.success">
      <div style="font-size: 45px;  color: #605c5c" >
      ...
      <div style="margin-top: 13px">
        ...
        <div style="font-size: 14px; color: gray">{{`${data.location.adm2} ${data.location.name}`}}</div>
      </div>
    </div>
    ...
    <div style="display: grid; grid-template-columns:repeat(5, 1fr); text-align: center;">
      <div v-for="item in data.hourly">
        <div>{{new Date(item.fxTime).getHours()}} 时</div>
        <div>
